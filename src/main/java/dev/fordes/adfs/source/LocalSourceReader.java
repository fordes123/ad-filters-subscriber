...
final class LocalSession implements SourceSession {
    ...
    LocalSession(String name, Path path, SourceSessionState state) {
        ...
        try {
            ...
            throw new InputException("无法解析本地来源目录: input=" + name + ", path=" + path, exception);
        }
    ...
    public SourceStream openInclude(SourceStream parent, String reference) {
        ...
        if (requested.isAbsolute()) {
            throw new InputException("include 只接受相对路径: reference=" + reference);
        }
        ...
        try {
            ...
            if (!includePath.startsWith(rootDirectory)) {
                throw new InputException("include 路径越过根来源目录: reference=" + reference);
            }
            return open(includePath, parent.includeDepth() + 1);
        } catch (IOException exception) {
            throw new InputException("无法解析 include 路径: reference=" + reference, exception);
        }
    }
    ...
    private SourceStream open(Path path, int depth) {
        ...
        if (!Files.isRegularFile(path) || !Files.isReadable(path)) {
            throw new InputException("本地输入不存在、不是普通文件或不可读: input=" + name + ", path=" + path);
        }
        try {
            ...
            throw new InputException("打开本地输入失败: input=" + name + ", path=" + path, exception);
        }
    }
